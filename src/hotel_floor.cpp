...
floor_::floor_(int flr, std::vector<room> rooms, int type) {
  ...
  // Code này là để mục đích để có thể filter cái loại phòng đó có bao nhiêu
  // phòng trong 1 tầng Ví dụ "type" SingleNormalCity thì có room.size() là 2
  Type[type] = rooms.size();
  num_rooms = Type[type];
}

int floor_::getNumRooms() { return num_rooms; }
...
void floor_::addRoom(room &r) {
  ...
  std::cout << "DEBUG: floor_::addRoom() called for room " << r.getID() << "\n";

  try {
    int roomType = r.getRoomTypeNumber();
    std::cout << "DEBUG: Room type: " << roomType << "\n";

    if (roomType >= 0 && roomType < 9) {
      rooms[roomType].push_back(r);
      Type[roomType] = 1;
      num_rooms++;
      std::cout << "DEBUG: Room added successfully to type " << roomType
                << "\n";
    } else {
      std::cout << "DEBUG: Invalid room type: " << roomType << "\n";
      throw std::runtime_error("Invalid room type: " +
                               std::to_string(roomType));
    }
  } catch (const std::exception &e) {
    std::cout << "DEBUG: Error in floor_::addRoom(): " << e.what() << "\n";
    throw;
  }
}
...
void floor_::displayAllFloorRooms() {
  for (int i = 1; i <= 8; i++)
    for (int j = 0; j < rooms[i].size(); j++) {
      ...
      std::cout << "State: "
                << ((rooms[i][j].isAvailable() == 1) ? "Available"
                                                     : "Not available")
                << ", ";
...
bool cmp(room a, room b) { return a.checkPrice() <= b.checkPrice(); }

// FIXING BUGS in SORTING ROOMS (!CMP IS WRONG)
std::vector<room> floor_::sortRoomsByPrice(bool ascend) {
  std::vector<room> ans;
  for (int i = 1; i <= 8; i++)
    for (int j = 0; j < rooms[i].size(); j++) ans.push_back(rooms[i][j]);
  if (ascend)
    std::sort(ans.begin(), ans.end(), cmp);
  else
    std::sort(ans.begin(), ans.end(),
              [](room &a, room &b) { return a.checkPrice() > b.checkPrice(); });
  return ans;
}
